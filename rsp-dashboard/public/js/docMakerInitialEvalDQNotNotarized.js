window.printInitialEvalDQNotNotarized = async function(id) {
    ...
    const reasonText = data.disqualificationReason || data.applicant?.disqualificationReason || 'Pursuant to Section 21 of DO 7 s. 2023 provides that "Individuals who failed to submit complete mandatory documents (Items 20.a to 20.j) on the set deadline indicated in the official memorandum shall not be included in the pool of official applicants.” and upon reviewing your submitted documents, your Omnibus Sworn Statement is not notarized.';
    ...
    const templateUrl = "/templates/Notice to DQ - Not notarized Omnibus.docx";
    const filename = `Initial_Eval_DQ_NotNotarized_${appName.replace(/[^a-zA-Z0-9]/g, '_')}.docx`;
