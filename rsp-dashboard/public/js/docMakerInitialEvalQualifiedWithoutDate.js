window.printInitialEvalQualifiedWithoutDate = async function(id) {
    ...
    const { d, dateStr } = await window.getOrSetDocDate(id, 'InitialEvalQualifiedWithoutDate', data);
    ...
    const templateUrl = "/templates/Notice to Qualified - Without Date of Assessment.docx";
    const filename = `Initial_Eval_Without_Date_${appName.replace(/[^a-zA-Z0-9]/g, '_')}.docx`;
