window.printAssignmentDivision = async function(id, name, office, dateStr, category, applicationCode, ccName, ccDesignation, reloadOnComplete = true) {
    ...
    const templateUrl = "/templates/Assignment Order - Division Office.docx";
    const filename = `Assignment_Order_Division_${name.replace(/[^a-zA-Z0-9]/g, '_')}.docx`;
    ...
    try {
        ...
        if (window.showToast) window.showToast('Assignment Letter DOC generated successfully.', 'success', reloadOnComplete);
    } catch (e) {
