window.printInitialEvalDQNoOmnibus = async function(id) {
    ...
    const { d, dateStr } = await window.getOrSetDocDate(id, 'InitialEvalDQNoOmnibus', data);
    ...
    const reasonText = data.disqualificationReason || data.applicant?.disqualificationReason || 'Pursuant to Section 21 of DO 7 s. 2023 provides that "Individuals who failed to submit complete mandatory documents (Items 20.a to 20.j) on the set deadline indicated in the official memorandum shall not be included in the pool of official applicants.” and upon reviewing your submitted documents, you failed to submit Omnibus Sworn Statement.';
    ...
    const templateUrl = "/templates/Notice to DQ - No Omnibus.docx";
    const filename = `Initial_Eval_DQ_No_Omnibus_${appName.replace(/[^a-zA-Z0-9]/g, '_')}.docx`;
