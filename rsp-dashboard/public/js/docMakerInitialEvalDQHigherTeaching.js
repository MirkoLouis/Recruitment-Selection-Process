window.printInitialEvalDQHigherTeaching = async function(id) {
    ...
    const { d, dateStr } = await window.getOrSetDocDate(id, 'InitialEvalDQHigherTeaching', data);
    ...
    const templateUrl = "/templates/Notice to DQ - Higher Teaching.docx";
    const filename = `Initial_Eval_DQ_Higher_Teaching_${appName.replace(/[^a-zA-Z0-9]/g, '_')}.docx`;
